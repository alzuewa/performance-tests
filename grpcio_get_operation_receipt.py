...
top_up_operation_request = MakeTopUpOperationRequest(
    status=OperationStatus.OPERATION_STATUS_COMPLETED,
    amount=fake.amount(),
    card_id=open_debit_card_account_response.account.cards[0].id,
    account_id=open_debit_card_account_response.account.id
)
top_up_operation_response: MakeTopUpOperationResponse = (
    operations_gateway_service.MakeTopUpOperation(top_up_operation_request)
)

get_operation_receipt_request = GetOperationReceiptRequest(operation_id=top_up_operation_response.operation.id)
